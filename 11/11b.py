...
from readInputB import readInput
fileName = argv[1]
...
class Monkey: # Wrong, these monkeys have no class.
	def __init__(self, id, items=[], operation=lambda x:x, testDivisor=None, monkeyIfTrue=None, monkeyIfFalse=None):
		self.activity = 0
		self.id = id
		self.items = items
		self.operation = operation
		self.howToTest = (testDivisor, monkeyIfTrue, monkeyIfFalse)
	...
	def inspectNextItem(self):
		...
		# item = int(item / 3)
		# OK, no, that didn't work! But we can get the remainder or division by the common divisor
		item = item % commonDivisor
		# Put back
		self.items.insert(0,item)

	def processNextItem(self):
		(testDivisor, monkeyIfTrue, monkeyIfFalse) = self.howToTest
		self.activity += 1
		item = self.items[0]

...
targetFree = 30000000
rootSize = tree['/'].getSize()
freeSize = totalFSSize - rootSize

smallestFileSize = totalFSSize + 1
for path in tree.keys():
	currNode = tree[path]
	if not currNode.isDir:
		continue
	size = currNode.getSize()
	if freeSize + size < targetFree: # not enough
		continue
	if size < smallestFileSize: # enough PLUS smallest!
		if (debug):
